...
class CategoryBudayaActivity : AppCompatActivity() {
    ...
    private lateinit var rv_category_budaya: RecyclerView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        rv_category_budaya= findViewById(R.id.rv_category_budaya)

        val layoutManager = GridLayoutManager(this, 2)
        rv_category_budaya.layoutManager = layoutManager
        fetchDataAttractionBudaya()

    }

    private fun fetchDataAttractionBudaya() {
        // ...

        CoroutineScope(Dispatchers.Main).launch {
            try {
                val response = apiService.getAttractionsCat1()
                if (response.status == "Success") {
                    val attractions = response.data
                    attractionAdapter = attractions?.let { AttractionAdapter(it) }!!
                    rv_category_budaya.adapter = attractionAdapter
                    ...
                    attractionAdapter.setOnItemClickListener(object : AttractionAdapter.OnItemClickListener {
                        override fun onItemClick(attraction: ModelAttraction) {
                            ...
                            val intent = Intent(this@CategoryBudayaActivity, DetailWisataActivity::class.java)
                    ...
                    rv_category_budaya.adapter
                } else {
                    Toast.makeText(this@CategoryBudayaActivity, "Failed to fetch data", Toast.LENGTH_SHORT)
                        .show()
                }
            } catch (e: Exception) {
                Toast.makeText(this@CategoryBudayaActivity, "Failed to fetch data", Toast.LENGTH_SHORT).show()
