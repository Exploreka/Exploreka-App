                ...
                if (response.status == "Success") {
                    ...
                    tourpackageAdapter.setOnItemClickListener(object : TourPackageAdapter.OnItemClickListener {
                        override fun onItemClick(tourpackage: ModelTourPackage) {
                            ...
                            val intent = Intent(this@PaketWisataMurahActivity, DetailPackageActivity::class.java)
                            intent.putExtra("tourpackageId", tourpackage.idTourPackage.toString() )
                            startActivity(intent)
                        }
                    })

                    rv_10_paket_wisata_murah.adapter
                } else {
                    Toast.makeText(this@PaketWisataMurahActivity, "Failed to fetch data", Toast.LENGTH_SHORT)
                        .show()
                }
            } catch (e: Exception) {
                Toast.makeText(this@PaketWisataMurahActivity, "Failed to fetch data", Toast.LENGTH_SHORT).show()
